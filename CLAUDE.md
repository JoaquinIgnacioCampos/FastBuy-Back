...
- Target: **Render** (free Docker web service) + **Neon Postgres** (replaces H2 in prod). Frontend is hosted separately on Cloudflare Pages and talks to this API **cross-origin via CORS** — backend routes stay at root (no `/api` prefix).
- `pom.xml`: swapped `mssql-jdbc` → `org.postgresql:postgresql` (runtime).
- `application.properties`: added `server.port=${PORT:8080}` (Render injects `PORT`).
- New `application-prod.properties` (activate with `SPRING_PROFILES_ACTIVE=prod`): Postgres driver + dialect, `spring.sql.init.mode=never`, `ddl-auto=none`, H2 console off. Datasource creds from env (`SPRING_DATASOURCE_URL/USERNAME/PASSWORD`).
- `Config/CorsConfig.java`: origins now read from `${fastbuy.cors-origins:http://localhost:5173}` — set `FASTBUY_CORS_ORIGINS` to the Cloudflare Pages origin in prod. Also set `FASTBUY_WEB_ORIGIN` to the frontend origin (MP/simulated return URL).
- New `Dockerfile` (multi-stage Maven build → JRE runtime) + `.dockerignore`. Backend-only image; the frontend is NOT bundled.
- New `deploy/postgres-init.sql`: one-time, idempotent (`CREATE TABLE IF NOT EXISTS` + `ON CONFLICT DO NOTHING`) Postgres port of `schema.sql` + `data.sql`. Converts `AUTO_INCREMENT`→`BIGINT GENERATED BY DEFAULT AS IDENTITY`, `DOUBLE`→`DOUBLE PRECISION`, `DATEADD`→`INTERVAL`; live events get a 365-day window; **no demo orders**, **no `p_test`**, **no bartender_users** (DataInitializer seeds those with BCrypt on first boot). Run once in Neon's SQL editor.
- Local dev + tests untouched (default profile still uses H2 + `schema.sql`/`data.sql`); all 66 tests stay green.

### 2026-06-22 — Order responses, no-show cancel, single-order lookup, claim window
...
- Merged into **production** on 2026-06-22 (this entry + the one above).
